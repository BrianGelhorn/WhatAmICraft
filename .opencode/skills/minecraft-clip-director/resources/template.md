...
Para una asignación, `clue_index` elige una pista y declarar `M_i` (matches por pista) y `V` (targets compatibles con ese clip). El validador exige solo `M_clue_index ⊆ V`; aún comprueba >=2 por cada `M_i`, reducción estricta al acumular las pistas 2 y 3, >=2 tras las dos primeras y un único target tras las tres. Esto comprueba conjuntos declarados, no interpreta el texto ni certifica semántica.
