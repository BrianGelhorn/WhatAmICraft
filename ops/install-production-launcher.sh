...
cd "$app_dir"
services="dashboard bot publisher-worker backup-rollback media"
for optional in clues-api analytics-api monitor; do
  if /usr/bin/docker compose --ansi never --project-directory "$app_dir" \
    --env-file "$env_file" -f "$app_dir/compose.yaml" config --services \
    | /usr/bin/grep -Fxq "$optional"; then
