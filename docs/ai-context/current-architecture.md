...
5. The dashboard handles review, queueing, publication, backups, and GPT exports; in staging, analytics is owned by `analytics-api`.

## Services and state

- Private dashboard: `https://what-am-i-craft.tail6cc348.ts.net:8443/`
- Public video base: `https://what-am-i-craft.tail6cc348.ts.net/`
- Docker services: `dashboard`, `bot`, `publisher-worker`, `media`; `clues-api` and `analytics-api` are currently staging-only.
