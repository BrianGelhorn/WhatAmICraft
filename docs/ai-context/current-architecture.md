...
3. `scripts/produce_quiz_copy.py` validates the episode, prepares audio, writes per-job props, renders `QuizCapasCopy`, renders the vertical thumbnail, and writes an immutable artifact manifest.
...
- Docker services: `dashboard`, `bot`, `publisher-worker`, `media`; `clues-api`, `analytics-api`, `monitor`, and `backup-rollback` are currently staging-only.
...
- `out/.active-template-version` controls which installed template release can enter automatic generation; `out/<video>.artifact.json` binds each video to its props, template version, and hashes.
