...
export const MysteryVideo: React.FC<{config?: MysteryVideoConfig}> = ({config = defaultMysteryConfig}) => {
  ...
  const preview = config.renderMode === "preview";
  ...
  return (
    <AbsoluteFill style={{overflow: "hidden", background: config.theme.mystery, fontFamily: config.theme.titleFont}}>
      ...
      <CanvasImage name="Moving background" src={staticFile(config.background)} style={{position: "absolute", inset: -50, width: 1180, height: 2020, objectFit: "cover", filter: preview ? "brightness(.24) saturate(.9)" : "brightness(.27) saturate(1.05)", transform: `translate(${drift * 0.35}px, ${-drift}px) scale(1.08)`}} />
      ...
      {(preview ? [0, 2, 4] : [0, 1, 2, 3, 4, 5, 6]).map((index) => <div key={index} style={{position: "absolute", left: 80 + index * 150 + Math.cos(loopPhase + index) * 24, top: 310 + index * 185 + Math.sin(loopPhase + index * 0.8) * 85, width: 5 + index, height: 5 + index, background: index % 2 ? config.theme.accent : config.theme.progress, opacity: 0.18, transform: "rotate(45deg)"}} />)}
