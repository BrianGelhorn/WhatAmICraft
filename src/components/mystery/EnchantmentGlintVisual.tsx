...
export const EnchantmentGlintVisual: React.FC<{
  ...
  return (
    <div style={{position: "relative", width: size * 1.65, height: size, display: "grid", placeItems: "center"}}>
      <div style={{position: "absolute", left: size * 0.02, width: size * 0.82, height: size * 0.82, borderRadius: "50%", background: `radial-gradient(circle, ${accentColor}48 0%, ${accentColor}18 42%, transparent 72%)`, filter: "blur(12px)", opacity: aura * 0.82, transform: `scale(${breathe})`}} />
      ...
      <CanvasImage src={staticFile(assetSrc)} style={{position: "absolute", left: size * 0.12 + swordShake, width: size * 0.68, height: size * 0.68, objectFit: "contain", imageRendering: "pixelated", filter: itemFilter, transform: `translateY(${Math.sin(cycle / 13) * 3}px) scale(${breathe + impact * 0.08})`}} />
