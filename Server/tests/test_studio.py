...
class StudioTests(unittest.TestCase):
    def test_source_and_generated_pack(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("Representa:", (functions / "scene/f02/setup_apply_actor.mcfunction").read_text())
            self.assertIn("durabilidad", (functions / "scene/f02/setup_apply_actor.mcfunction").read_text())
            self.assertIn("Hacé:", (functions / "scene/g11/setup_apply_actor.mcfunction").read_text())
            self.assertIn("160 80 8", (functions / "scene/f03/setup_apply_actor.mcfunction").read_text())
            self.assertIn("166.5 81 9.5", (functions / "scene/f03/setup_apply_actor.mcfunction").read_text())
            ...
            self.assertIn("execute as @a at @s in minecraft:overworld", (functions / "scene/g11/setup_apply.mcfunction").read_text())
            self.assertFalse((functions / "claim.mcfunction").exists())
            self.assertFalse((functions / "admin/release.mcfunction").exists())
            self.assertIn("studio_chunk", (functions / "load.mcfunction").read_text())
