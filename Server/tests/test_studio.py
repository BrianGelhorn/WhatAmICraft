...
class StudioTests(unittest.TestCase):
    def test_source_and_generated_pack(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("minecraft:ladder[facing=west]", (functions / "scene/f02/setup_apply_actor.mcfunction").read_text())
            self.assertIn("minecraft:oak_planks", (functions / "scene/f03/setup_apply_actor.mcfunction").read_text())
            self.assertIn("minecraft:sea_lantern", (functions / "scene/f04/setup_apply_actor.mcfunction").read_text())
            ...
            self.assertEqual(next_scene.count("return run function studio:scene/"), 6)
