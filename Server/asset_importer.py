...
def _legacy_state(block_id: int, data: int) -> tuple[str, dict[str, str]]:
    simple = {
        1: "stone", 2: "grass_block", 3: "dirt", 4: "cobblestone", 12: "sand",
        13: "gravel", 20: "glass", 24: "sandstone", 30: "cobweb", 48: "mossy_cobblestone",
        49: "obsidian", 78: "snow", 80: "snow_block", 82: "clay", 87: "netherrack",
        88: "soul_sand", 89: "glowstone", 98: "stone_bricks", 108: "brick_stairs",
        109: "stone_brick_stairs", 121: "end_stone", 155: "quartz_block", 159: "white_terracotta",
        172: "terracotta", 174: "packed_ice",
    }
...
def find_source() -> Path | None:
    ...
    candidates = sorted(ROOT.parent.glob(SOURCE_PATTERN))
...
def import_bundle(out: Path) -> list[dict]:
    ...
    with zipfile.ZipFile(source) as archive:
        for info in archive.infolist():
            ...
            # Duplicate names are impossible in this bundle; keep a stable suffix if a future bundle collides.
            if target.exists():
                suffix = 2
            ...
            catalog.append({"id": asset_id, "family": family, "variant": variant, "source": Path(info.filename).name, "filename_blocks": filename_count, **meta, "tags": ["mushroom", "organic", "bundle"], "scenes": ["f03", "f09", "f10", "f11"]})
    catalog.sort(key=lambda item: item["id"])
    (out / "asset_catalog.json").write_text(json.dumps({"source": source.name, "count": len(catalog), "assets": catalog}, indent=2, sort_keys=True), encoding="utf-8")
    return catalog
...
def placement_commands(scene: str, seed: int, origin: tuple[int, int], catalog: list[dict], height_fn, structure_root: Path | None = None) -> tuple[list[str], dict | None]:
    if scene != "f11" or not catalog:
        return [], None
    preferred = ["blue_bells", "little_grey", "pointy_boletus", "death_trumpet", "calocera_cornea", "round_boletus", "red_amanita", "morels"]
    selected = []
    for family in preferred:
        candidates = [item for item in catalog if item["family"] == family]
        if candidates:
            selected.append(candidates[(seed + len(selected)) % len(candidates)])
